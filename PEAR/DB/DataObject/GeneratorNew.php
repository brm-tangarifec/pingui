...
/**
...

    {
        ...
        $perms = file_exists($file) ? fileperms($file) : 0777;
	...
	**/
    ...
    function _generateDefinitionsTable()
    {
        ...
        foreach($defs as $t) {
			...
			$fk = $this->_fkeys;
				 ...
				 if (preg_match("/(multiple_key|nextval\()/i",rawurldecode($t->flags))) {
					  if ($write_ini) {
							$keys_out_fkey .= "{$t->name} = N\n";
						}
						foreach($fk as $table => $details) {
							foreach ($details as $col => $ref) {
								if($col == $t->name){
									$ret_keys_fkey[$t->name] = $ref;
								}
							}
						}							
				 }
		...
		$ret['fkeys'] = empty($keys_out_fkey)?'': $ret_keys_fkey;
    ...
    function generateClasses()
    {
		...
		$fIni = fopen("{$base}/requires.ini.php", "w");
		 fwrite($fIni,"<?php\n");
        foreach($this->tables as $this->table) {
            $this->table = trim($this->table);
            
            $tempName = split("_",$this->table);
            $tempArr = array();
            $i=0;
            $firstWord = $fileName = "";
            ...
            foreach( $tempName as $key => $value ){
            	if( $key == 0 ){
            		...
            		$firstWord = $firstWord.ucwords($value);
            	}
            	$tempArr[$i] = ucwords($value);
          ...
          //  $this->classname = $class_prefix.preg_replace('/_*[^A-Z0-9]/i','_',ucwords($tempName));
             $this->classname = $class_prefix.$tempName;
           // $this->classname = $class_prefix.preg_replace('/[^A-Z0-9]/i','_',ucfirst($this->table));
            ...
            if (strpos($options['class_location'],'%s') !== false) {
               ...
               // $outfilename = "{$base}/".preg_replace('/[^A-Z0-9]/i','_',ucfirst($this->table)).".php";
                $outfilename = "{$base}/db.$fileName.php";
           		//$tempName
            }
            $oldcontents = '';
            ...
            fwrite($fIni,"require(\"./\".\$prefijo.\"db/db.$fileName.php\");\n");
    ...
    function _generateClassTable($input = '')
    {
        ...
        $body .= "\n";
        $body .= "    /* Static get */\n";
        $body .= "    function &staticGet(\$class,\$k,\$v=NULL) { return DB_DataObject::staticGet('{$this->classname}',\$k,\$v); }\n";
        ...
        */
        ...
        if (!empty($options['generator_no_ini'])) {
			...
			$body .= $this->_generateForeignKeysFunction($def['fkeys']);
    ...
    */
    function _generateKeysFunction($def) 
    {
	 ...
	 /**
    * Generate Foreign keys Function - used generator_no_ini is set.
    *
    * @param    array  keys array.
    * @return   string
    * @access   public
    */
    function _generateForeignKeysFunction($def) 
    {
        $ret = "\n";
		if($def)
		{
